namespace NToolbox.Globalization
{
	/// <summary>
	/// Contains three-letter ISO 639-2/B bibliographic language codes defined in ISO 639-2/B.
	/// </summary>
	public static class LanguageAlpha3B
	{
		/// <summary>
		/// Abkhaz (Code: abk)
		/// </summary>
		public const string Abkhaz = "abk";

		/// <summary>
		/// Afar (Code: aar)
		/// </summary>
		public const string Afar = "aar";

		/// <summary>
		/// Afrikaans (Code: afr)
		/// </summary>
		public const string Afrikaans = "afr";

		/// <summary>
		/// Akan (Code: aka)
		/// </summary>
		public const string Akan = "aka";

		/// <summary>
		/// Albanian (Code: alb)
		/// </summary>
		public const string Albanian = "alb";

		/// <summary>
		/// Amharic (Code: amh)
		/// </summary>
		public const string Amharic = "amh";

		/// <summary>
		/// Arabic (Code: ara)
		/// </summary>
		public const string Arabic = "ara";

		/// <summary>
		/// Aragonese (Code: arg)
		/// </summary>
		public const string Aragonese = "arg";

		/// <summary>
		/// Armenian (Code: arm)
		/// </summary>
		public const string Armenian = "arm";

		/// <summary>
		/// Assamese (Code: asm)
		/// </summary>
		public const string Assamese = "asm";

		/// <summary>
		/// Australian (Code: ast)
		/// </summary>
		public const string Australian = "ast";

		/// <summary>
		/// Avaric (Code: ava)
		/// </summary>
		public const string Avaric = "ava";

		/// <summary>
		/// Avestan (Code: ave)
		/// </summary>
		public const string Avestan = "ave";

		/// <summary>
		/// Aymara (Code: aym)
		/// </summary>
		public const string Aymara = "aym";

		/// <summary>
		/// Azerbaijani (Code: aze)
		/// </summary>
		public const string Azerbaijani = "aze";

		/// <summary>
		/// Bambara (Code: bam)
		/// </summary>
		public const string Bambara = "bam";

		/// <summary>
		/// Bashkir (Code: bak)
		/// </summary>
		public const string Bashkir = "bak";

		/// <summary>
		/// Basque (Code: baq)
		/// </summary>
		public const string Basque = "baq";

		/// <summary>
		/// Belarusian (Code: bel)
		/// </summary>
		public const string Belarusian = "bel";

		/// <summary>
		/// Bengali, Bangla (Code: ben)
		/// </summary>
		public const string Bengali = "ben";

		/// <summary>
		/// Bihari (Code: bih)
		/// </summary>
		public const string Bihari = "bih";

		/// <summary>
		/// Bislama (Code: bis)
		/// </summary>
		public const string Bislama = "bis";

		/// <summary>
		/// Bosnian (Code: bos)
		/// </summary>
		public const string Bosnian = "bos";

		/// <summary>
		/// Breton (Code: bre)
		/// </summary>
		public const string Breton = "bre";

		/// <summary>
		/// Bulgarian (Code: bul)
		/// </summary>
		public const string Bulgarian = "bul";

		/// <summary>
		/// Burmese (Code: bur)
		/// </summary>
		public const string Burmese = "bur";

		/// <summary>
		/// Catalan (Code: cat)
		/// </summary>
		public const string Catalan = "cat";

		/// <summary>
		/// Chamorro (Code: cha)
		/// </summary>
		public const string Chamorro = "cha";

		/// <summary>
		/// Chechen (Code: che)
		/// </summary>
		public const string Chechen = "che";

		/// <summary>
		/// Chichewa, Chewa, Nyanja (Code: nya)
		/// </summary>
		public const string Chichewa = "nya";

		/// <summary>
		/// Chinese (Code: chi)
		/// </summary>
		public const string Chinese = "chi";

		/// <summary>
		/// Chuvash (Code: chv)
		/// </summary>
		public const string Chuvash = "chv";

		/// <summary>
		/// Cornish (Code: cor)
		/// </summary>
		public const string Cornish = "cor";

		/// <summary>
		/// Corsican (Code: cos)
		/// </summary>
		public const string Corsican = "cos";

		/// <summary>
		/// Cree (Code: cre)
		/// </summary>
		public const string Cree = "cre";

		/// <summary>
		/// Croatian (Code: hrv)
		/// </summary>
		public const string Croatian = "hrv";

		/// <summary>
		/// Czech (Code: cze)
		/// </summary>
		public const string Czech = "cze";

		/// <summary>
		/// Danish (Code: dan)
		/// </summary>
		public const string Danish = "dan";

		/// <summary>
		/// Divehi, Dhivehi, Maldivian (Code: div)
		/// </summary>
		public const string Divehi = "div";

		/// <summary>
		/// Dutch (Code: dut)
		/// </summary>
		public const string Dutch = "dut";

		/// <summary>
		/// Dzongkha (Code: dzo)
		/// </summary>
		public const string Dzongkha = "dzo";

		/// <summary>
		/// English (Code: eng)
		/// </summary>
		public const string English = "eng";

		/// <summary>
		/// Esperanto (Code: epo)
		/// </summary>
		public const string Esperanto = "epo";

		/// <summary>
		/// Estonian (Code: est)
		/// </summary>
		public const string Estonian = "est";

		/// <summary>
		/// Ewe (Code: ewe)
		/// </summary>
		public const string Ewe = "ewe";

		/// <summary>
		/// Faroese (Code: fao)
		/// </summary>
		public const string Faroese = "fao";

		/// <summary>
		/// Fijian (Code: fij)
		/// </summary>
		public const string Fijian = "fij";

		/// <summary>
		/// Finnish (Code: fin)
		/// </summary>
		public const string Finnish = "fin";

		/// <summary>
		/// French (Code: fre)
		/// </summary>
		public const string French = "fre";

		/// <summary>
		/// Fula, Fulah, Pulaar, Pular (Code: ful)
		/// </summary>
		public const string Fula = "ful";

		/// <summary>
		/// Galician (Code: glg)
		/// </summary>
		public const string Galician = "glg";

		/// <summary>
		/// Georgian (Code: geo)
		/// </summary>
		public const string Georgian = "geo";

		/// <summary>
		/// German (Code: ger)
		/// </summary>
		public const string German = "ger";

		/// <summary>
		/// Greek (modern) (Code: gre)
		/// </summary>
		public const string Greek = "gre";

		/// <summary>
		/// Guaraní (Code: grn)
		/// </summary>
		public const string Guaraní = "grn";

		/// <summary>
		/// Gujarati (Code: guj)
		/// </summary>
		public const string Gujarati = "guj";

		/// <summary>
		/// Haitian, Haitian Creole (Code: hat)
		/// </summary>
		public const string Haitian = "hat";

		/// <summary>
		/// Hausa (Code: hau)
		/// </summary>
		public const string Hausa = "hau";

		/// <summary>
		/// Hebrew (modern) (Code: heb)
		/// </summary>
		public const string Hebrew = "heb";

		/// <summary>
		/// Herero (Code: her)
		/// </summary>
		public const string Herero = "her";

		/// <summary>
		/// Hindi (Code: hin)
		/// </summary>
		public const string Hindi = "hin";

		/// <summary>
		/// Hiri Motu (Code: hmo)
		/// </summary>
		public const string HiriMotu = "hmo";

		/// <summary>
		/// Hungarian (Code: hun)
		/// </summary>
		public const string Hungarian = "hun";

		/// <summary>
		/// Interlingua (Code: ina)
		/// </summary>
		public const string Interlingua = "ina";

		/// <summary>
		/// Indonesian (Code: ind)
		/// </summary>
		public const string Indonesian = "ind";

		/// <summary>
		/// Interlingue (Code: ile)
		/// </summary>
		public const string Interlingue = "ile";

		/// <summary>
		/// Irish (Code: gle)
		/// </summary>
		public const string Irish = "gle";

		/// <summary>
		/// Igbo (Code: ibo)
		/// </summary>
		public const string Igbo = "ibo";

		/// <summary>
		/// Inupiaq (Code: ipk)
		/// </summary>
		public const string Inupiaq = "ipk";

		/// <summary>
		/// Ido (Code: ido)
		/// </summary>
		public const string Ido = "ido";

		/// <summary>
		/// Icelandic (Code: ice)
		/// </summary>
		public const string Icelandic = "ice";

		/// <summary>
		/// Italian (Code: ita)
		/// </summary>
		public const string Italian = "ita";

		/// <summary>
		/// Inuktitut (Code: iku)
		/// </summary>
		public const string Inuktitut = "iku";

		/// <summary>
		/// Japanese (Code: jpn)
		/// </summary>
		public const string Japanese = "jpn";

		/// <summary>
		/// Javanese (Code: jav)
		/// </summary>
		public const string Javanese = "jav";

		/// <summary>
		/// Kalaallisut, Greenlandic (Code: kal)
		/// </summary>
		public const string Kalaallisut = "kal";

		/// <summary>
		/// Kannada (Code: kan)
		/// </summary>
		public const string Kannada = "kan";

		/// <summary>
		/// Kanuri (Code: kau)
		/// </summary>
		public const string Kanuri = "kau";

		/// <summary>
		/// Kashmiri (Code: kas)
		/// </summary>
		public const string Kashmiri = "kas";

		/// <summary>
		/// Kazakh (Code: kaz)
		/// </summary>
		public const string Kazakh = "kaz";

		/// <summary>
		/// Khmer (Code: khm)
		/// </summary>
		public const string Khmer = "khm";

		/// <summary>
		/// Kikuyu, Gikuyu (Code: kik)
		/// </summary>
		public const string Kikuyu = "kik";

		/// <summary>
		/// Kinyarwanda (Code: kin)
		/// </summary>
		public const string Kinyarwanda = "kin";

		/// <summary>
		/// Kyrgyz (Code: kir)
		/// </summary>
		public const string Kyrgyz = "kir";

		/// <summary>
		/// Komi (Code: kom)
		/// </summary>
		public const string Komi = "kom";

		/// <summary>
		/// Kongo (Code: kon)
		/// </summary>
		public const string Kongo = "kon";

		/// <summary>
		/// Korean (Code: kor)
		/// </summary>
		public const string Korean = "kor";

		/// <summary>
		/// Kurdish (Code: kur)
		/// </summary>
		public const string Kurdish = "kur";

		/// <summary>
		/// Kwanyama, Kuanyama (Code: kua)
		/// </summary>
		public const string Kwanyama = "kua";

		/// <summary>
		/// Latin (Code: lat)
		/// </summary>
		public const string Latin = "lat";

		/// <summary>
		/// Luxembourgish, Letzeburgesch (Code: ltz)
		/// </summary>
		public const string Luxembourgish = "ltz";

		/// <summary>
		/// Ganda (Code: lug)
		/// </summary>
		public const string Ganda = "lug";

		/// <summary>
		/// Limburgish, Limburgan, Limburger (Code: lim)
		/// </summary>
		public const string Limburgish = "lim";

		/// <summary>
		/// Lingala (Code: lin)
		/// </summary>
		public const string Lingala = "lin";

		/// <summary>
		/// Lao (Code: lao)
		/// </summary>
		public const string Lao = "lao";

		/// <summary>
		/// Lithuanian (Code: lit)
		/// </summary>
		public const string Lithuanian = "lit";

		/// <summary>
		/// Luba-Katanga (Code: lub)
		/// </summary>
		public const string LubaKatanga = "lub";

		/// <summary>
		/// Latvian (Code: lav)
		/// </summary>
		public const string Latvian = "lav";

		/// <summary>
		/// Manx (Code: glv)
		/// </summary>
		public const string Manx = "glv";

		/// <summary>
		/// Macedonian (Code: mac)
		/// </summary>
		public const string Macedonian = "mac";

		/// <summary>
		/// Malagasy (Code: mlg)
		/// </summary>
		public const string Malagasy = "mlg";

		/// <summary>
		/// Malay (Code: may)
		/// </summary>
		public const string Malay = "may";

		/// <summary>
		/// Malayalam (Code: mal)
		/// </summary>
		public const string Malayalam = "mal";

		/// <summary>
		/// Maltese (Code: mlt)
		/// </summary>
		public const string Maltese = "mlt";

		/// <summary>
		/// Māori (Code: mao)
		/// </summary>
		public const string Maori = "mao";

		/// <summary>
		/// Marathi (Marāṭhī) (Code: mar)
		/// </summary>
		public const string Marathi = "mar";

		/// <summary>
		/// Marshallese (Code: mah)
		/// </summary>
		public const string Marshallese = "mah";

		/// <summary>
		/// Mongolian (Code: mon)
		/// </summary>
		public const string Mongolian = "mon";

		/// <summary>
		/// Nauru (Code: nau)
		/// </summary>
		public const string Nauru = "nau";

		/// <summary>
		/// Navajo, Navaho (Code: nav)
		/// </summary>
		public const string Navajo = "nav";

		/// <summary>
		/// Northern Ndebele (Code: nde)
		/// </summary>
		public const string NorthernNdebele = "nde";

		/// <summary>
		/// Nepali (Code: nep)
		/// </summary>
		public const string Nepali = "nep";

		/// <summary>
		/// Ndonga (Code: ndo)
		/// </summary>
		public const string Ndonga = "ndo";

		/// <summary>
		/// Norwegian Bokmål (Code: nob)
		/// </summary>
		public const string NorwegianBokmal = "nob";

		/// <summary>
		/// Norwegian Nynorsk (Code: nno)
		/// </summary>
		public const string NorwegianNynorsk = "nno";

		/// <summary>
		/// Norwegian (Code: nor)
		/// </summary>
		public const string Norwegian = "nor";

		/// <summary>
		/// Nuosu (Code: iii)
		/// </summary>
		public const string Nuosu = "iii";

		/// <summary>
		/// Southern Ndebele (Code: nbl)
		/// </summary>
		public const string SouthernNdebele = "nbl";

		/// <summary>
		/// Occitan (Code: oci)
		/// </summary>
		public const string Occitan = "oci";

		/// <summary>
		/// Ojibwe, Ojibwa (Code: oji)
		/// </summary>
		public const string Ojibwe = "oji";

		/// <summary>
		/// Old Church Slavonic, Church Slavonic, Old Bulgarian (Code: chu)
		/// </summary>
		public const string OldChurchSlavonic = "chu";

		/// <summary>
		/// Oromo (Code: orm)
		/// </summary>
		public const string Oromo = "orm";

		/// <summary>
		/// Oriya (Code: ori)
		/// </summary>
		public const string Oriya = "ori";

		/// <summary>
		/// Ossetian, Ossetic (Code: oss)
		/// </summary>
		public const string Ossetian = "oss";

		/// <summary>
		/// Panjabi, Punjabi (Code: pan)
		/// </summary>
		public const string Panjabi = "pan";

		/// <summary>
		/// Pāli (Code: pli)
		/// </summary>
		public const string Pali = "pli";

		/// <summary>
		/// Persian (Farsi) (Code: per)
		/// </summary>
		public const string Persian = "per";

		/// <summary>
		/// Polish (Code: pol)
		/// </summary>
		public const string Polish = "pol";

		/// <summary>
		/// Pashto, Pushto (Code: pus)
		/// </summary>
		public const string Pashto = "pus";

		/// <summary>
		/// Portuguese (Code: por)
		/// </summary>
		public const string Portuguese = "por";

		/// <summary>
		/// Quechua (Code: que)
		/// </summary>
		public const string Quechua = "que";

		/// <summary>
		/// Romansh (Code: roh)
		/// </summary>
		public const string Romansh = "roh";

		/// <summary>
		/// Kirundi (Code: run)
		/// </summary>
		public const string Kirundi = "run";

		/// <summary>
		/// Romanian (Code: rum)
		/// </summary>
		public const string Romanian = "rum";

		/// <summary>
		/// Russian (Code: rus)
		/// </summary>
		public const string Russian = "rus";

		/// <summary>
		/// Sanskrit (Saṁskṛta) (Code: san)
		/// </summary>
		public const string Sanskrit = "san";

		/// <summary>
		/// Sardinian (Code: srd)
		/// </summary>
		public const string Sardinian = "srd";

		/// <summary>
		/// Sindhi (Code: snd)
		/// </summary>
		public const string Sindhi = "snd";

		/// <summary>
		/// Northern Sami (Code: sme)
		/// </summary>
		public const string NorthernSami = "sme";

		/// <summary>
		/// Samoan (Code: smo)
		/// </summary>
		public const string Samoan = "smo";

		/// <summary>
		/// Sango (Code: sag)
		/// </summary>
		public const string Sango = "sag";

		/// <summary>
		/// Serbian (Code: srp)
		/// </summary>
		public const string Serbian = "srp";

		/// <summary>
		/// Scottish Gaelic, Gaelic (Code: gla)
		/// </summary>
		public const string Gaelic = "gla";

		/// <summary>
		/// Shona (Code: sna)
		/// </summary>
		public const string Shona = "sna";

		/// <summary>
		/// Sinhala, Sinhalese (Code: sin)
		/// </summary>
		public const string Sinhala = "sin";

		/// <summary>
		/// Slovak (Code: slo)
		/// </summary>
		public const string Slovak = "slo";

		/// <summary>
		/// Slovene (Code: slv)
		/// </summary>
		public const string Slovene = "slv";

		/// <summary>
		/// Somali (Code: som)
		/// </summary>
		public const string Somali = "som";

		/// <summary>
		/// Southern Sotho (Code: sot)
		/// </summary>
		public const string SouthernSotho = "sot";

		/// <summary>
		/// Spanish (Code: spa)
		/// </summary>
		public const string Spanish = "spa";

		/// <summary>
		/// Sundanese (Code: sun)
		/// </summary>
		public const string Sundanese = "sun";

		/// <summary>
		/// Swahili (Code: swa)
		/// </summary>
		public const string Swahili = "swa";

		/// <summary>
		/// Swati (Code: ssw)
		/// </summary>
		public const string Swati = "ssw";

		/// <summary>
		/// Swedish (Code: swe)
		/// </summary>
		public const string Swedish = "swe";

		/// <summary>
		/// Tamil (Code: tam)
		/// </summary>
		public const string Tamil = "tam";

		/// <summary>
		/// Telugu (Code: tel)
		/// </summary>
		public const string Telugu = "tel";

		/// <summary>
		/// Tajik (Code: tgk)
		/// </summary>
		public const string Tajik = "tgk";

		/// <summary>
		/// Thai (Code: tha)
		/// </summary>
		public const string Thai = "tha";

		/// <summary>
		/// Tigrinya (Code: tir)
		/// </summary>
		public const string Tigrinya = "tir";

		/// <summary>
		/// Tibetan Standard, Tibetan, Central (Code: tib)
		/// </summary>
		public const string Tibetan = "tib";

		/// <summary>
		/// Turkmen (Code: tuk)
		/// </summary>
		public const string Turkmen = "tuk";

		/// <summary>
		/// Tagalog (Code: tgl)
		/// </summary>
		public const string Tagalog = "tgl";

		/// <summary>
		/// Tswana (Code: tsn)
		/// </summary>
		public const string Tswana = "tsn";

		/// <summary>
		/// Tonga (Tonga Islands) (Code: ton)
		/// </summary>
		public const string Tonga = "ton";

		/// <summary>
		/// Turkish (Code: tur)
		/// </summary>
		public const string Turkish = "tur";

		/// <summary>
		/// Tsonga (Code: tso)
		/// </summary>
		public const string Tsonga = "tso";

		/// <summary>
		/// Tatar (Code: tat)
		/// </summary>
		public const string Tatar = "tat";

		/// <summary>
		/// Twi (Code: twi)
		/// </summary>
		public const string Twi = "twi";

		/// <summary>
		/// Tahitian (Code: tah)
		/// </summary>
		public const string Tahitian = "tah";

		/// <summary>
		/// Uyghur (Code: uig)
		/// </summary>
		public const string Uyghur = "uig";

		/// <summary>
		/// Ukrainian (Code: ukr)
		/// </summary>
		public const string Ukrainian = "ukr";

		/// <summary>
		/// Urdu (Code: urd)
		/// </summary>
		public const string Urdu = "urd";

		/// <summary>
		/// Uzbek (Code: uzb)
		/// </summary>
		public const string Uzbek = "uzb";

		/// <summary>
		/// Venda (Code: ven)
		/// </summary>
		public const string Venda = "ven";

		/// <summary>
		/// Vietnamese (Code: vie)
		/// </summary>
		public const string Vietnamese = "vie";

		/// <summary>
		/// Volapük (Code: vol)
		/// </summary>
		public const string Volapük = "vol";

		/// <summary>
		/// Walloon (Code: wln)
		/// </summary>
		public const string Walloon = "wln";

		/// <summary>
		/// Welsh (Code: wel)
		/// </summary>
		public const string Welsh = "wel";

		/// <summary>
		/// Wolof (Code: wol)
		/// </summary>
		public const string Wolof = "wol";

		/// <summary>
		/// Western Frisian (Code: fry)
		/// </summary>
		public const string WesternFrisian = "fry";

		/// <summary>
		/// Xhosa (Code: xho)
		/// </summary>
		public const string Xhosa = "xho";

		/// <summary>
		/// Yiddish (Code: yid)
		/// </summary>
		public const string Yiddish = "yid";

		/// <summary>
		/// Yoruba (Code: yor)
		/// </summary>
		public const string Yoruba = "yor";

		/// <summary>
		/// Zhuang, Chuang (Code: zha)
		/// </summary>
		public const string Zhuang = "zha";

		/// <summary>
		/// Zulu (Code: zul)
		/// </summary>
		public const string Zulu = "zul";
	}
}